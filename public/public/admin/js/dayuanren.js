$(function () {
        ...
        if ((target = $(this).attr('href')) || (target = $(this).attr('url'))) {
            if ($(this).hasClass('prompt')) {
                layer.prompt({title: $(that).attr('prompt-title'), formType: 2}, function (text, index) {
                    layer.close(index);
                    target += "?&prompt_remark=" + text;
                    ajaxSubGet(that, target);
                });
            } else {
                ajaxSubGet(that, target);
            }
    ...
    function ajaxSubGet(that, target) {
    ...
    $('.ajax-post').click(function () {
        ...
        if (($(this).attr('type') == 'submit') || (target = $(this).attr('href')) || (target = $(this).attr('url'))) {
            ...
            if ($(this).attr('hide-data') === 'true') {//无数据时也可以使用的功能
                ...
                var ids = checkboxValues.join(',');
                if (nead_confirm && $(this).hasClass('confirm')) {
                    if (!confirm('确认要执行该操作吗?')) {
                        return false;
                    }
                }
                if (nead_confirm && $(this).hasClass('prompt') && $(this).hasClass('batch_create_time')) {
                    layer.prompt({title: $(that).attr('prompt-title'), formType: 0}, function (text, index) {
                        layer.close(index);
                        query += "&create_time=" + text;
                        ajaxSubPost(that, target, query);
                    });
                    return false;
                }
                if (nead_confirm && $(this).hasClass('prompt') && $(this).hasClass('batch_finish_time')) {
                    layer.prompt({title: $(that).attr('prompt-title'), formType: 0}, function (text, index) {
                        layer.close(index);
                        query += "&finish_time=" + text;
                        ajaxSubPost(that, target, query);
                    });
                    return false;
                }
                if (nead_confirm && $(this).hasClass('prompt') && $(this).hasClass('batch_api')) {
