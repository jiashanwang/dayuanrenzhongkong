...
class Wtltdwl
{
    ...
    public function recharge($out_trade_num, $mobile, $param, $isp = '')
    {
		...
		if($param['oparam3']=="大连市" and $this->mchid==10145) $area="大连";
    ...
    public function selfcheck($param)
    {
        ...
        if ($res['errno'] == 0 && isset($res['data']['data']['status']) && $res['data']['data']['partner_id'] == $partner_id) {
            if ($res['data']['data']['status'] == 2) {
                PorderModel::rechargeSusApi('wtltdwl', $param['api_order_number'], $res['data'], "完成充值:" . $res['data']['data']['charge_amount']);
                ...
                if ($res['data']['data']['charge_amount'] != $res['data']['data']['amount'] && $res['data']['data']['charge_amount'] > 0) {
                    //充值进度变化
                    PorderModel::rechargeRateApi('wtltdwl', $param['api_order_number'], $res['data'], $res['data']['data']['charge_amount']);
                }
            }
        }
    }
    ...
    public function notify()
    {
        if (I('selfcheck')) {
            ...
            $porders = M('porder p')
                ...
                ->where(['r.callapi' => 'Wtltdwl', 'p.status' => 3])
            ...
            foreach ($porders as $k => $order) {
                ...
                if ($res['errno'] == 0 && isset($res['data']['data']['status'])) {
                    if ($res['data']['data']['status'] == 2) {
                        PorderModel::rechargeSusApi('wtltdwl', $order['api_order_number'], $res['data'], "|完成充值:" . $res['data']['data']['charge_amount']);
                    } elseif (in_array($res['data']['data']['status'], [-1, 1])) {
                        if ($res['data']['data']['charge_amount'] != $res['data']['data']['amount'] && $res['data']['data']['charge_amount'] > 0) {
            ...
            $porders = M('porder p')
                ->join('reapi r', 'r.id=p.api_cur_id')
                ->where(['r.callapi' => 'Wtltdwl', 'p.status' => 3, 'apply_refund' => 1])
            ...
            if ($state == 2) {
                PorderModel::rechargeSusApi('wtltdwl', I('partner_order_no'), $_POST, "完成充值:" . I('charge_amount'));
                echo "success";
            } elseif (in_array($state, [-1, 0, 1, 2])) {
                if ($charge_amount == 0) {
					if(I('remarks')=='手动取消11'){
						PorderModel::rechargeFailApi('wtltdwl', I('partner_order_no'), $_POST);
					}
					else{
						PorderModel::rechargeFailApi('wtltdwl', I('partner_order_no'), $_POST, I('remarks'));
					}
                } else {
                    PorderModel::rechargePartApi('wtltdwl', I('partner_order_no'), $_POST, I('remarks') . "|部分充值：" . I('charge_amount'));
                }
                echo "success";
            }
        }
