...
class Menu extends Admin
{
	public function index()
	{
		...
		$mns = M('menu')->where(array('module' => $module))->order('sort,id')->select();
	...
	public function proc_menu_Html($tree, $lv = 0)
	{
		...
		foreach ($tree as $t) {
			...
			if ($t['child']) {
				...
				$html .= "<a class=\"ajax-get confirm\" href=\"" . url($del) . "\"><span style=\"float: right;margin-right: 10px;color: #b31a2f;\" onclick=\"return del()\">删除</span></a>" . "<a class='open-window no-refresh' title='添加下级' href=\"" . url($addnew) . "\"><span class=\"text-navy\" style=\"float: right\">添加下级&nbsp;</span></a>" . " </li>" . "<ul class=\"list-group\" style='display: none;'>";
				...
				$html .= "<span  class=\"badge badge-warning\">" . $t['sort'] . "</span>";
				$html .= "<a class=\"ajax-get confirm\" url=\"" . url($del) . "\"><span style=\"float: right;margin-right: 10px;color: #b31a2f;\">删除</span></a>" . "<a class='open-window no-refresh' title='添加下级' href=\"" . url($addnew) . "\"><span class=\"text-navy\" style=\"float: right\">添加下级&nbsp;</span></a>" . " </li>";
			}
		}
		return $html;
	}
	public function add($id = 0)
	{
		if (request()->isPost()) {
			...
			$menus = M('Menu')->field(true)->where(array('module' => $module))->order('sort asc')->select();
