...
class AuthGroup extends \app\common\model\AuthGroup
{
	public function get_auth($module)
	{
		...
		foreach ($list as $key => $vo) {
			if ($module == 'admin') {
				$list[$key]['mem_count'] = M('auth_group_access a')->join("member m", "m.id=a.user_id")->where(array('a.group_id' => $vo['id'], 'm.is_del' => 0))->count();
			} else {
				$list[$key]['mem_count'] = M('auth_group_access a')->join("customer m", "m.id=a.user_id")->where(array('a.group_id' => $vo['id'], 'm.is_del' => 0))->count();
			}
		}
		return $list;
	}
	...
	{
		$list = M('auth_group')->where(array('module' => $module))->select();
		return $list;
	}
}